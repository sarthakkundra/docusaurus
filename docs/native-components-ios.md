...
- commandArgs:(NSArrayid \*)commandArgs  -  Args of the native method that we can pass from JS to native.
...
Here the `is defined in the` file and contains only one parameter which is `. This exported function will find a particular view using` which contains the `parameter and returns the component based on` allowing it to call the method on the correct component.

## Styles

Since all our native react views are subclasses of `, most style attributes will work like you would expect out of the box. Some components will want a default style, however, for example` which is a fixed size. This default style is important for the layout algorithm to work as expected, but we also want to be able to override the default style when using the component. `` does this by wrapping the native component in an extra view, which has flexible styling, and using a fixed style (which is generated with constants passed in from native) on the inner native component:

```jsx
```

The `` constants are exported from native by grabbing the actual frame of the native component like so:
