...
`controls how children are measured and displayed.` causes views to be clipped while `causes views to be measured independently of their parents main axis. It works like` in CSS (default: visible). See for more details. `` only works on iOS. On Android, all views will clip their children.
