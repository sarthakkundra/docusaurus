...
If `rationale` is provided, this function checks with the OS whether it is necessary to show a dialog explaining why the permission is needed (https:) and then shows the system permission dialog.
...
| rationale  | object | No       | Object with a `and`.       |
