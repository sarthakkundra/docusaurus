...
* Having same name headers on Android will result in only the latest one being present. A temporary solution can be found here: https:.
* Cookie based authentication is currently unstable. You can view some of the issues raised here:
* As a minimum on iOS, when redirected through a `, if a` header is present, the cookie is not set properly. Since the redirect cannot be handled manually this might cause a scenario where infinite requests occur if the redirect is the result of an expired session.
