...
&gt; Please note that the terminology around what integration testing means is not always consistent. Also, the line between what is a unit test and what is an integration test may not always be clear. For this guide, your test falls into "integration testing" if it: &gt; &gt; - Uses an external system (such as weather service API) &gt; - Makes a network call to other application &gt; - Does any kind of file or database <abbr title="Input/Output">I/O</abbr>
