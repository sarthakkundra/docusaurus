...
The config object provides enhanced versions of all of the responder callbacks that provide not only the typical `, but also the` gesture state, by replacing the word `with` in each of the typical `callbacks. For example, the` object would look like:
