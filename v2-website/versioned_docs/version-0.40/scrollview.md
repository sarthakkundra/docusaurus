...
return ( scrollview ); ... const styles = StyleSheet.create({"{"} contentContainer: {"{"} paddingVertical: 20 {"}"} {"}"});
...
- `default` (the default), same as `black`. alex ignore rule
- `black`, scroll indicator is black. This style is good against a light content background. alex ignore rule
